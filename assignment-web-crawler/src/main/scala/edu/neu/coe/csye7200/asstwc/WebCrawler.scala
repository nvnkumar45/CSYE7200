...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    // 16 points.
    def getURLs(ns: Node): Seq[Try[URL]] = for (n <- ns \\ "a") yield createURL(Option(u), n \@ "href") // TO BE IMPLEMENTED
    ...
    // 9 points.
    for {
      content <- getURLContent(u)
      future <- MonadOps.asFuture(getLinks(content))
    } yield future
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
    Future.sequence(for(a <- for(t <- us2) yield wget(t)) yield MonadOps.sequence(a))
  }
