...
object Function {
  ...
  /**
    ...
    */
  def map2[T1, T2, R](t1y: Try[T1], t2y: Try[T2])(f: (T1, T2) => R): Try[R] = for{ t1 <- t1y
                                                                                   t2 <- t2y} yield f(t1,t2) // TO BE IMPLEMENTED
  ...
  /**
    ...
    * @return a value of R, wrapped in Try
    */
  def map3[T1, T2, T3, R](t1y: Try[T1], t2y: Try[T2], t3y: Try[T3])(f: (T1, T2, T3) => R): Try[R] = for{ t1 <- t1y
                                                                                                         t2 <- t2y
                                                                                                         t3 <- t3y} yield f(t1,t2,t3) // TO BE IMPLEMENTED

  /**
    * You get the idea...
    */
  def map7[T1, T2, T3, T4, T5, T6, T7, R](t1y: Try[T1], t2y: Try[T2], t3y: Try[T3], t4y: Try[T4], t5y: Try[T5], t6y: Try[T6], t7y: Try[T7])
                                         (f: (T1, T2, T3, T4, T5, T6, T7) => R): Try[R] = for{ t1 <- t1y
                                                                                               t2 <- t2y
                                                                                               t3 <- t3y
                                                                                               t4 <- t4y
                                                                                               t5 <- t5y
                                                                                               t6 <- t6y
                                                                                               t7 <- t7y} yield f(t1,t2,t3,t4,t5,t6,t7) // TO BE IMPLEMENTED
  ...
  def invert3[T1, T2, T3, R](f: T1 => T2 => T3 => R): T3 => T2 => T1 => R = {
    def invert(z:T3)(y:T2)(x:T1):R = f(x)(y)(z)
    invert
  } // TO BE IMPLEMENTED
  ...
  def invert4[T1, T2, T3, T4, R](f: T1 => T2 => T3 => T4 => R): T4 => T3 => T2 => T1 => R = {
    def invert(t4:T4)(t3:T3)(t2:T2)(t1:T1):R = f(t1)(t2)(t3)(t4)
    invert
  } // TO BE IMPLEMENTED
